...
# Everything runs inside this script block, in a scope of its own. `irm | iex`
# evaluates the text in the caller's session: at the top level, every variable
# below — and `$ErrorActionPreference = 'Stop'` — would outlive the install and
# change how the user's next command fails. The parameters are handed in rather
# than read from the enclosing scope, so the block reads the same either way.
& {
  param([string] $Version, [string] $Dir, [switch] $NoModifyPath)

  $ErrorActionPreference = 'Stop'
  # Windows PowerShell 5.1 redraws its progress bar for every chunk
  # Invoke-WebRequest receives, which makes a few-megabyte download take minutes.
  # Scoped to this block like the line above, so the user's own preference is
  # back the moment the installer returns.
  ...
  # like a network problem. This one setting is process-wide and cannot be
  # scoped, so it is added to what the session allows, never put in its place.
  [Net.ServicePointManager]::SecurityProtocol =
    [Net.ServicePointManager]::SecurityProtocol -bor [Net.SecurityProtocolType]::Tls12

  $Repo = 'pro100andrey/flutter_redux_templates'
  if (-not $Dir) { $Dir = Join-Path $env:LOCALAPPDATA 'frx\bin' }
  # Absolute, against PowerShell's current location — [IO.Path]::GetFullPath
  # would resolve against the process's, which `cd` in PowerShell does not move.
  # The directory goes into the user PATH, where a relative one means nothing.
  $Dir = $ExecutionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath($Dir)

  # `throw`, not `exit`: under `irm | iex` the script runs in the user's own
  # session, and `exit` would close their PowerShell window along with the
  # error. A throw ends the installer, prints the message, and — run as a file —
  # still exits nonzero.
  function Fail([string] $Message) { throw "frx: $Message" }
  ...
  try {
    ...
    # The user PATH as stored, read and written through the registry.
    # [Environment]::GetEnvironmentVariable hands back the value *expanded*, and
    # SetEnvironmentVariable writes a plain string: a round trip through them
    # turned every `%USERPROFILE%\…` entry into today's literal path, for good —
    # and an entry naming another variable stopped following it. Read
    # unexpanded, written back as REG_EXPAND_SZ, the entries this installer did
    # not add come out exactly as they went in.
    ...
    try {
      ...
      $onPath = @($userPath -split ';' | Where-Object {
        $_ -and [Environment]::ExpandEnvironmentVariables($_).TrimEnd('\') -ieq $want
      }).Count -gt 0
      ...
      if ($onPath) {
        ...
        # The *user* PATH, never the machine one: this install is per-user and
        # does not run elevated. Read-modify-write of the User value only —
        # expanding and rewriting the whole of $env:Path would bake the machine
        # entries into the user's, which survives uninstalling whatever put them
        # there.
        $updated = if ([string]::IsNullOrEmpty($userPath)) { $Dir } else { "$($userPath.TrimEnd(';'));$Dir" }
        $envKey.SetValue('Path', $updated, [Microsoft.Win32.RegistryValueKind]::ExpandString)
        # A registry write tells no one. SetEnvironmentVariable is what
        # broadcasts WM_SETTINGCHANGE, so that Explorer — and every terminal it
        # starts from here on — rereads the environment; setting and clearing a
        # variable of our own is that broadcast without touching anything real.
        [Environment]::SetEnvironmentVariable('FRX_INSTALLER', '1', 'User')
        [Environment]::SetEnvironmentVariable('FRX_INSTALLER', $null, 'User')
...
} -Version $Version -Dir $Dir -NoModifyPath:$NoModifyPath
