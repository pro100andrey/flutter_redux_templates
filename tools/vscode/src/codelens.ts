...
export class FrxLensProvider implements vscode.CodeLensProvider {
  ...
  provideCodeLenses(document: vscode.TextDocument): vscode.CodeLens[] {
    ...
    const state = file.match(
      new RegExp(`\\${sep}redux\\${sep}([a-z0-9_]+)\\${sep}models\\${sep}\\1_state\\.dart$`),
    );
    if (state) {
    ...
    const connector = file.match(new RegExp(`\\${sep}connectors\\${sep}(\\w+)_page_connector\\.dart$`));
    if (connector) {
      const className = `${naming.pascalOf(connector[1])}PageConnector`;
      const page = path.join(this.root, 'ui', 'lib', 'pages', `${connector[1]}_page.dart`);
      return [
    ...
    const page = file.match(new RegExp(`\\${sep}pages\\${sep}(\\w+)_page\\.dart$`));
    if (page) {
      const conn = path.join(this.root, 'app', 'lib', 'connectors', `${page[1]}_page_connector.dart`);
