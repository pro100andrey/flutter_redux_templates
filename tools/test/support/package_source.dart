...
Directory? packageLibOf(String package, {Directory? repoRoot}) {
  final config = _packageConfigFor(package, repoRoot ?? Directory.current);
  if (config == null) {
    return null;
  }
  ...
  for (final entry in (json['packages'] as List? ?? const [])) {
    ...
    // rootUri is relative to the .dart_tool directory when it is not
    // absolute.
...
Iterable<File> packageDartFiles(Directory dir) => dir
...
File? _packageConfigFor(String package, Directory start) {
