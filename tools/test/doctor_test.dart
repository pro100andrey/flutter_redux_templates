...
void main() {
  ...
  /// The previews mirror drifts silently: nothing imports `lib/previews/`, so
  /// a widget moved by hand leaves its preview behind, still compiling.
  group('the previews mirror', () {
    void putWidget(String relative) => fx.file('ui/lib/$relative')
      ..parent.createSync(recursive: true)
      ..writeAsStringSync('// widget\n');

    void putPreview(String relative) => fx.file('ui/lib/previews/$relative')
      ..parent.createSync(recursive: true)
      ..writeAsStringSync('// preview\n');

    test('a preview whose widget moved is reported', () async {
      putPreview('inputs/pin_form_field.dart');
      // The widget now lives elsewhere — the mirror was not updated.
      putWidget('fields/pin_form_field.dart');

      final fs = await findings();
      final drift = fs.firstWhere(
        (f) => '${f['message']}'.contains('pin_form_field'),
        orElse: () => {},
      );
      expect(drift['severity'], 'warn', reason: fs.toString());
      expect(drift['fix'], isNull, reason: 'delete or move — frx cannot pick');
      expect('${drift['message']}', contains('ui/lib/inputs/pin_form_field'));
    });

    test('a preview beside its widget is not', () async {
      putWidget('inputs/pin_form_field.dart');
      putPreview('inputs/pin_form_field.dart');

      final fs = await findings();
      expect(
        fs.every((f) => !'${f['message']}'.contains('previews a widget')),
        isTrue,
        reason: fs.toString(),
      );
    });
    ...
    test(
      'a widget with no preview is not — that is coverage, not drift',
      () async {
        putWidget('inputs/pin_form_field.dart');
        // A previews tree exists, so the check is active; this widget has none.
        putPreview('buttons/button.dart');
        putWidget('buttons/button.dart');

        final fs = await findings();
        expect(
          fs.every((f) => !'${f['message']}'.contains('pin_form_field')),
          isTrue,
          reason: fs.toString(),
        );
      },
    ...
    test('a shared file at the previews root is not an orphan', () async {
      // `previews/defaults.dart` — sample values every preview draws on. The
      // mirror is previews/<dir>/<file>, so depth 0 reflects nothing and must
      // not be read as a preview whose widget went missing.
      putPreview('defaults.dart');
      final fs = await findings();
      expect(
        fs.every((f) => !'${f['message']}'.contains('defaults.dart')),
        isTrue,
        reason: fs.toString(),
      );
    });

    test('no previews tree at all keeps the check quiet', () async {
      putWidget('inputs/pin_form_field.dart');
      final fs = await findings();
      expect(
        fs.every((f) => !'${f['message']}'.contains('previews')),
        isTrue,
        reason: fs.toString(),
      );
    });
