...
/// The record `--json` emits per mixin. Named rather than spelled inline
/// because the field list has no spaces to break the help string on.
const _jsonShape = '{name,clause,summary,implies,conflictsWith,swallowsAfter}';
...
class ListMixinsCommand extends Command<int> {
  ListMixinsCommand() {
    argParser
      ..addFlag(
        'json',
        negatable: false,
        help: 'Emit JSON ({mixins:[$_jsonShape]}) instead of a table.',
      )
  ...
  Future<int> run() async {
    ...
    for (final m in ActionMixin.values) {
      ...
      // implies says what has to precede this one — neither says which mixin
      // silently eats the other's cleanup.
      const swallows =
    ...
    if (project.isNotEmpty) {
      ...
      for (final m in project) {
        ...
        for (final h in m.hooks) {
          final chaining = h.chainsSuper
              ? 'chains super — safe anywhere in the clause'
              : '**does not chain** — it must come LAST, and nothing that '
