...
class VmField {
  ...
  ///
  /// A convention is incomplete by construction:
  /// `typedef OnTap = void Function()` in somebody's project is not recognised,
  /// and its field would be reported. That is the reason this finding is a
  /// silenceable warning rather than an error — and the reason the rule was
  /// widened once already, when `FormFieldValidator<T>?` on this repository's
  /// own `FieldVm` was reported and the file next to it explained, in a
  /// comment, exactly why it is correct.
  bool get isCallback {
...
class ViewModel {
  ...
  /// The names this class compares on, as bare identifiers in its equality
  /// list.
  ///
  ...
  /// `super(equals: [\u2026])`, which is what every view-model in this
  /// repository actually uses. Reading only the first is why the check below
  /// had never fired here and could not have.
  ///
  /// Empty is a real answer \u2014 `equals: [1]` compares nothing \u2014 and is
  /// no longer the same as [equalityReadable] being false. Conflating them is
  /// what let one element that changes nothing about `==` switch the rule off
  /// for a class.
...
/// not required: `readViewModels` is also pointed at the shared models
/// (`FieldVm`, `ChoiceVm`), and at whatever a hand-written widget uses.
List<ViewModel> readViewModels(String source) =>
    _viewModelsIn(parseString(content: source, throwIfDiagnostics: false).unit);
...
List<ViewModel> viewModelsOfFile(File file) =>
...
ViewModel? _readViewModelClass(ClassDeclaration decl) {
  ...
  return ViewModel(
    ...
    fields: [
      for (final p in ctor.parameters.parameters) ?_readParameter(p, declared),
    ],
...
/// `ids.length` can be reported as the partial comparison it is rather than
/// as an absence — the field is spelled right there, and calling it missing
/// reads as the tool being wrong.
_Equality _identifiersIn(Expression expr) {
