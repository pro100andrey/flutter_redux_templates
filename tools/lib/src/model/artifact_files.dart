...
/// `models/lib/<snake>.dart` — where `add-modelFile` and `add-enum` both write.
///
/// One method for two commands because it is one path: a freezed modelFile and
/// a plain enum are one file in one directory, and `remove --kind modelFile`
/// covers both for exactly that reason.
String modelFile(FrxWorkspace repo, Casing name) =>
    p.join(repo.modelsLib.path, '${name.snake}.dart');

/// The build_runner output beside a [modelFile] — `<snake>.freezed.dart`,
...
List<String> modelGeneratedFiles(FrxWorkspace repo, Casing name) {
...
Directory serviceDir(FrxWorkspace repo, Casing name) => Directory(
...
({String service, String dispatcher}) serviceFiles(
  FrxWorkspace repo,
  Casing name,
) {
...
///
/// No removal reads it — `remove` has no `retrofitFile` kind — and it is here
