...
- Mixins conflict, and the conflict does **not** fail the build: `with
  NonReentrant, Throttle` compiles and analyzes clean, and is caught by an
  `assert` inside async_redux on the first dispatch — in a debug build
  only. Ask `frx list-mixins` which exclude which and let the scaffolder
  write the `with` clause: refusing the pair up front is the only check
